...
const ChatView: React.FC = () => {
  ...
  const renderAvatar = (role: AgentRole) => {
    switch (role) {
      case 'manager':
        return <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center text-white shadow-md shadow-blue-500/20"><UserCog className="w-4 h-4" /></div>;
      case 'strategist':
        return <div className="w-8 h-8 rounded-lg bg-purple-600 flex items-center justify-center text-white shadow-md shadow-purple-500/20"><TrendingUp className="w-4 h-4" /></div>;
      case 'scraper':
        return <div className="w-8 h-8 rounded-lg bg-emerald-600 flex items-center justify-center text-white shadow-md shadow-emerald-500/20"><Database className="w-4 h-4" /></div>;
      default:
        return <div className="w-8 h-8 rounded-lg bg-slate-200 flex items-center justify-center text-slate-600"><User className="w-4 h-4" /></div>;
    }
  };

  const getBubbleStyle = (role: AgentRole) => {
    switch (role) {
      case 'user': return 'bg-slate-900 text-white rounded-tr-none';
      case 'manager': return 'bg-blue-50 border border-blue-100 text-slate-800 rounded-tl-none';
      case 'strategist': return 'bg-purple-50 border border-purple-100 text-slate-800 rounded-tl-none';
      case 'scraper': return 'bg-emerald-50 border border-emerald-100 text-slate-800 rounded-tl-none';
      ...
      <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          <div className="flex -space-x-2">
             <div className="w-8 h-8 rounded-full border-2 border-white bg-blue-600 flex items-center justify-center text-white text-[10px] font-bold" title="Sarah (Manager)">S</div>
             <div className="w-8 h-8 rounded-full border-2 border-white bg-purple-600 flex items-center justify-center text-white text-[10px] font-bold" title="Marcus (Strategist)">M</div>
             <div className="w-8 h-8 rounded-full border-2 border-white bg-emerald-600 flex items-center justify-center text-white text-[10px] font-bold" title="Alex (Scraper)">A</div>
          </div>
          <div>
            <h3 className="font-bold text-slate-900 text-sm">Agent Team Active</h3>
            <p className="text-xs text-slate-500">Collaborating on your strategy</p>
          </div>
        </div>
        <div className="px-2 py-1 bg-green-50 text-green-700 text-[10px] font-bold uppercase tracking-wider rounded border border-green-100 flex items-center gap-1">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar bg-slate-50/30">
                  ...
                  {msg.hasAttachment && !msg.isStreaming && (
                    <div className="mt-4 pt-4 border-t border-black/10">
                       <button 
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-white rounded-lg border border-slate-200 hover:bg-slate-50 hover:border-slate-300 text-slate-700 text-xs font-bold transition-all shadow-sm w-full justify-center"
                       >
        ...
        {isStreaming && messages.length > 0 && messages[messages.length-1].role === 'user' && (
           <div className="flex gap-4">
             ...
             <div className="bg-white border border-slate-100 rounded-2xl rounded-tl-none px-5 py-3.5 shadow-sm">
      ...
      <div className="p-4 bg-white border-t border-slate-100 shrink-0">
        <form onSubmit={handleSubmit} className="relative">
          <input
            ...
            placeholder="Describe your business or approve the plan..."
            disabled={isStreaming}
            className="w-full pl-4 pr-12 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all outline-none text-sm font-medium text-slate-900 placeholder:text-slate-400 shadow-inner"
          />
          <button
            type="submit"
        ...
        <div className="text-center mt-2">
          <p className="text-[10px] text-slate-400">Agents are simulating search & outreach. Verify generated data.</p>
        </div>
      </div>
    </div>
  );
